...
import { register as apiRegister, API_URL } from '../../services/api';
...
function Register() {
    ...
    useEffect(() => {
        ...
        if (errorParam === 'oauth_failed') {
            ...
            setError(`${provider} OAuth is not configured. Please use email/password registration or contact the administrator.`);
        } else if (errorParam === 'leetcode_unavailable') {
            setError('LeetCode OAuth is not available. Please use another login method.');
        }

        // Fetch OAuth status
        fetch(`${API_URL}/auth/oauth-status`)
            .then(res => res.json())
        ...
        <div className="container" style={{ maxWidth: '400px', marginTop: '100px' }}>
            <div className="card">
                ...
                <div style={{ display: 'flex', alignItems: 'center', margin: '24px 0' }}>
                    ...
                    <span style={{ padding: '0 16px', color: 'var(--text-secondary)', fontSize: '14px' }}>OR</span>
                    <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--border-color)' }}></div>
                </div>
                ...
                <div style={{ display: 'grid', gap: '12px' }}>
                    ...
                    {!oauthStatus.google && !oauthStatus.github && !oauthStatus.linkedin && (
                        <div style={{
                            padding: '16px',
                            ...
                            <p style={{ margin: 0 }}>
                                Social login is not configured. Please use email/password to register.
                            </p>
                        </div>
                    )}
                </div>
